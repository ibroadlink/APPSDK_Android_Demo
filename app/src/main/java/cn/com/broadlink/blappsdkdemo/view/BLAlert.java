...
public class BLAlert {
    ...
    public static Dialog showDilog(Context context, int titleId, int messageId, int positiveButtonNameId, int negativeButtonNameId, final DialogOnClickListener clickListener) {
        return showDilog(context, titleId != 0 ? context.getString(titleId) : null, messageId != 0 ? context.getString(messageId) : null, positiveButtonNameId != 0 ?
                context.getString(positiveButtonNameId) : null, negativeButtonNameId != 0 ? context.getString(negativeButtonNameId) : null, clickListener);
     ...
     * @param title              标题
     * @param message            信息
     * @param positiveButtonName 确认按钮名称
     * @param negativeButtonName 取消按钮名称
     * @param clickListener      按钮时间
     * @return AlertDialog
     * 对话框
     */
    public static Dialog showDilog(Context context, String title, String message, String positiveButtonName, String negativeButtonName, final DialogOnClickListener clickListener) {
        return BLStyleDialog.Builder(context).setTitle(title).setMessage(message).setCacelButton(negativeButtonName, new BLStyleDialog.OnBLDialogBtnListener() {
            @Override
    ...
    public static Dialog showDialog(Context context, String title, String message, final DialogOnClickListener clickListener) {
        return BLStyleDialog.Builder(context).setTitle(title).setMessage(message).setCacelButton("Cancel", new BLStyleDialog.OnBLDialogBtnListener() {
            @Override
            public void onClick(Button btn) {
                if (clickListener != null) {
                    clickListener.onNegativeClick();
    ...
    public static Dialog showAlert(Context context, String title, String message, final OnSingleClickListener clickListener) {
        return BLStyleDialog.Builder(context).setTitle(title).setMessage(message).setConfimButton("Ok", new BLStyleDialog.OnBLDialogBtnListener() {
            @Override
            public void onClick(Button btn) {
                if (clickListener != null) {
    ...
    public static AlertDialog showCustomViewDilog(Context context, View view, String positiveButtonName, String negativeButtonName, final DialogOnClickListener clickListener) {
        final Builder builder = new Builder(context, AlertDialog.THEME_HOLO_LIGHT).setView(view).setCancelable(false);
     ...
     * @param context            上下文
     * @param title              标题
     * @param contentHint        提示内容
     * @param value              EidtText需要显示的内容信息
     * @param editHint           EidtText hint需要显示的内容信息
     * @param positiveButtonName 确认按钮名称
     * @param negativeButtonName 取消按钮名称
     * @param clickListener      按钮时间
     * @return AlertDialog
     * 对话框
     */
    public static AlertDialog showEditDilog(final Context context, String title, String contentHint, String value, String editHint, String positiveButtonName,
     ...
     * 密码输入的alert
     */
    public static AlertDialog showPwdEditDilog(final Context context, String title, String contentHint, String value, String editHint, String positiveButtonName,
    ...
    public static Dialog showAlert(Context context, String title, String messageId, String confimButtonText, BLStyleDialog.OnBLDialogBtnListener confimBtnListener,
                                   String cancleButtonText, BLStyleDialog.OnBLDialogBtnListener cacenlBtnListener) {
        return BLStyleDialog.Builder(context).setTitle(title).setMessage(messageId).setCacelButton(cancleButtonText, cacenlBtnListener).setConfimButton(confimButtonText,
                confimBtnListener).show();
    }

    public static Dialog showAlert(final Context context, final String title, View view, final BLStyleDialog.OnBLDialogBtnListener confimBtnListener) {
        
        final Dialog dlg = new Dialog(context, R.style.MMTheme_DataSheet);
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        RelativeLayout layout = (RelativeLayout) inflater.inflate(R.layout.view_bl_bottom_alert_content, null);
        ...
        FrameLayout mFlContent = (FrameLayout) layout.findViewById(R.id.fl_content);
        ...
        mTvOk.setOnClickListener(new OnSingleClickListener() {
            ...
            public void doOnClick(View v) {
                if(confimBtnListener != null){
                    confimBtnListener.onClick(null);
                    dlg.dismiss();
                }
            }
        });
