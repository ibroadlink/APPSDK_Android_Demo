 ...
 * <p>
 * <p>You can also add items to TabLayout in your layout through the use of {@link BLTabItem}.
 ...
 *
 *     &lt;android.support.design.widget.BLTabItem
 ...
 *
 * 基于{@link android.support.design.widget.TabLayout} 修改可以自定义文字大小与下标宽度
 */
@ViewPager.DecorView
public class BLTabLayout extends HorizontalScrollView {
    ...
    //当Tab被选中时文本长度大于等于Tab的宽度时，Tab会另外增加SELECT_TAB_SELECTED_ADD_WIDTH的长度
    ...
    /**
     * Gravity used to fill the {@link BLTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link BLTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    float mTabTextSize = 0;
    float mTabSelectedTextSize = 0;
    float mTabTextMultiLineSize;

    //文本字母是否小写转大写
    boolean mTabTextAllCaps = false;
    //下标长度是否随TextView长度变化
    boolean mTabIndicatorWidthWithText = false;
    ...
    public BLTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BLTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.BLTabLayout,
                defStyleAttr, R.style.BLTabLayout);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.BLTabLayout_tabIndicatorHeight, 0));
        //设置下标宽度
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.BLTabLayout_tabIndicatorWidth, dpToPx(20)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.BLTabLayout_tabIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.BLTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAllCaps = a.getBoolean(R.styleable.BLTabLayout_tabTextAllCaps, mTabTextAllCaps);

        mTabIndicatorWidthWithText = a.getBoolean(R.styleable.BLTabLayout_tabIndicatorWidthWithText, mTabIndicatorWidthWithText);

        mTabTextAppearance = a.getResourceId(R.styleable.BLTabLayout_tabTextAppearance,
        ...
        try {
            mTabTextSize = ta.getDimensionPixelSize(
                    android.support.design.R.styleable.TextAppearance_android_textSize, 0);
            mTabTextColors = ta.getColorStateList(
                    android.support.design.R.styleable.TextAppearance_android_textColor);
        } finally {
            ta.recycle();
        }

        //设置字体大小
        if (a.hasValue(R.styleable.BLTabLayout_tabTextSize)){
            mTabTextSize = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabTextSize, 0);
        }

        if (a.hasValue(R.styleable.BLTabLayout_tabSelectedTextSize)){
            mTabSelectedTextSize = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabSelectedTextSize, 0);
        }

        if (a.hasValue(R.styleable.BLTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.BLTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.BLTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.BLTabLayout_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.BLTabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.BLTabLayout_tabContentStart, 0);
        mMode = a.getInt(R.styleable.BLTabLayout_tab_gravity, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.BLTabLayout_tab_gravity, GRAVITY_FILL);
        a.recycle();
    ...
    private void addTabFromItemView(@NonNull BLTabItem item) {
    ...
    /**
     * Returns the current mode used by this {@link BLTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link BLTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    private void addViewInternal(final View child) {
        if (child instanceof BLTabItem) {
            addTabFromItemView((BLTabItem) child);
        } else {
            throw new IllegalArgumentException("Only BLTabItem instances can be added to TabLayout");
        }
    }
    ...
    private void ensureScrollAnimator() {
        if (mScrollAnimator == null) {
            ...
            mScrollAnimator.setInterpolator(BLAnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
    ...
    class TabView extends LinearLayout {
        ...
        private void updateTextAndIcon(@Nullable final TextView textView,
            ...
            if (textView != null) {
                if (hasText) {
                    textView.setAllCaps(mTabTextAllCaps);
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mSelectedIndicatorWidth == 0
                        && !mTabIndicatorWidthWithText) mSelectedIndicatorWidth = getTabMaxWidth();
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                ...
                animator.setInterpolator(BLAnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        setIndicatorPosition(
                                BLAnimationUtils.lerp(startLeft, targetLeft, fraction),
                                BLAnimationUtils.lerp(startRight, targetRight, fraction));
                    }
                });
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                if (mSelectedIndicatorWidth != 0 && !mTabIndicatorWidthWithText) {
                    int maxWidth = mIndicatorRight - mIndicatorLeft;
                    if (maxWidth > mSelectedIndicatorWidth) {
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        ...
        // We return the default layout params so that we don't blow up if we're given a BLTabItem
     ...
     * necessary calls back to the provided {@link BLTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<BLTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(BLTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final BLTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
     ...
     */
    public static class ViewPagerOnTabSelectedListener implements BLTabLayout.OnTabSelectedListener {
